...
更新源码后请重启后端；开发模式重启 Vite，单地址模式先重新执行 `npm run build --prefix frontend`。浏览器用 `Ctrl+F5` 加载最新页面，保留站点数据即可迁移原有访问身份和会话记录。

| 现象 | 排查 |
|---|---|
| 切换项目实例后上传或历史异常 | 前端会按后端数据环境恢复访问身份；确认前后端均为最新源码，重启服务并 `Ctrl+F5`。无需清除站点数据。 |
| 看不到原历史会话 | 确认仍使用原浏览器地址和原运行数据目录。不同数据环境的历史独立保存；切回原环境可复用已保存的凭据，清除站点数据后无法仅靠会话 ID 找回访问权。 |
| API 工具返回 `401/403` | 调用 `POST /api/auth/anonymous` 获取当前环境的 Bearer，并在后续请求中携带 `Authorization: Bearer <token>`；已有有效 Bearer 可用于复用原身份。 |
